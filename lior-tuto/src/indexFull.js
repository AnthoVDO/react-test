//This is the same page as index.js but all on a single page
    ...
    render(){
        ...
        <div>
           ...
           {/*<button onClick={this.handelClick}>Click me</button>*/}
            <ul>
                {this.state.customers.map(x=>(<li key={x.id} >{x.name}<button onClick={()=>{this.handleDelete(x.id)}}>X</button></li>))}
            </ul>
